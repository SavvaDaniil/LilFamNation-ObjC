...
-(void)closeView {
    ...
    [self dismissViewControllerAnimated:NO completion:^{ [[NSNotificationCenter defaultCenter] postNotificationName:@"exitChangePassword" object:self];
    }];
}
...
-(IBAction)clickBtnSave:(id)sender {
    if([password.text isEqualToString:@""]){
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Ошибка" message:@"Введите пожалуйста новый пароль" preferredStyle:UIAlertControllerStyleAlert];
...
-(void)changePassword {
    ...
    if (error_profile) {
        ...
        if (jsonError != nil) {
            ...
            NSURL *url_profile = [NSURL URLWithString:[NSString stringWithFormat:@"%@/script_for_change_password.php",[GlobalVariables sharedInstance].global_url]];
            NSMutableURLRequest *urlRequest_profile = [NSMutableURLRequest requestWithURL:url_profile];
            [urlRequest_profile setHTTPMethod:@"POST"];
            NSString *userUpdate_profile = [NSString stringWithFormat:@"a=%@&b=%@&password=%@",id_of_user, check_hash, getPassword];
            ...
            if (error_profile) {
                ...
                if (jsonError_profile != nil) {
                    ...
                    if([[jsonDict_profile objectForKey:@"error"] isEqualToString:@"1"]){

                        
                    } else if([[jsonDict_profile objectForKey:@"error"] isEqualToString:@"0"]) {
                        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Успешно" message:@"Пароль успешно изменён" preferredStyle:UIAlertControllerStyleAlert];
                        UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"Закрыть" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
                                [self closeView];
                        }];
                        [alert addAction:okAction];
