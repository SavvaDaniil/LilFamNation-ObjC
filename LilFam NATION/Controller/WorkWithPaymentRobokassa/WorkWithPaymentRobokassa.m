...
#import "WorkWithPaymentRobokassa.h"
...
@interface WorkWithPaymentRobokassa ()

@end

@implementation WorkWithPaymentRobokassa
@synthesize mHTMLReportAll;

- (void)viewDidLoad {
    [super viewDidLoad];
    mHTMLReportAll.scrollView.bounces = NO;
    
    NSLog(@"date_of_action = %@",[GlobalVariables sharedInstance].global_date_of_action);
}
-(void)viewDidAppear:(BOOL)animated{
    [super viewDidAppear:YES];
    [self user_begin_payment];
}
...
-(IBAction)return:(id)sender {
    ...
    transition.subtype = kCATransitionFromLeft;
    [self.view.window.layer addAnimation:transition forKey:nil];
    [self dismissViewControllerAnimated:NO completion:^{ [[NSNotificationCenter defaultCenter] postNotificationName:@"succeessPaymentRobokassa" object:self];
    }];
}

-(void)user_begin_payment {
    ...
    if (error_profile) {
        ...
        if (jsonError != nil) {
            ...
            NSURL *url = [NSURL URLWithString: @"https://lilfam.com/application/user_begin_payment.php"];
            NSString *body = [NSString stringWithFormat: @"a=%@&b=%@&id_of_group=%@&id_base_of_abonement=%@&date_of_action=%@",id_of_user, check_hash, [GlobalVariables sharedInstance].global_id_of_group , [GlobalVariables sharedInstance].g_id_base_of_abonement, [GlobalVariables sharedInstance].global_date_of_action];
            NSLog(@"user_begin_payment = %@",body);
