...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(exitChangePassword:)  name:@"exitChangePassword" object:nil];
}
- (void) exitChangePassword:(NSNotification *) notification
{
    if ([[notification name] isEqualToString:@"exitChangePassword"])
        [self closeView];
}
...
-(IBAction)clickBtnSave:(id)sender {
    if([password.text isEqualToString:@""]){
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Ошибка" message:@"Введите пожалуйста пароль" preferredStyle:UIAlertControllerStyleAlert];
...
-(void)checkPassword {
    ...
    if (error_profile) {
        ...
        if (jsonError != nil) {
            ...
            NSURL *url_profile = [NSURL URLWithString:[NSString stringWithFormat:@"%@/script_for_check_password.php",[GlobalVariables sharedInstance].global_url]];
            NSMutableURLRequest *urlRequest_profile = [NSMutableURLRequest requestWithURL:url_profile];
            [urlRequest_profile setHTTPMethod:@"POST"];
            ...
            if (error_profile) {
                ...
                if (jsonError_profile != nil) {
                    ...
                    if([[jsonDict_profile objectForKey:@"error"] isEqualToString:@"1"]){
                        ...
                        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Ошибка" message:@"Неправильный пароль" preferredStyle:UIAlertControllerStyleAlert];
                        UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"Закрыть" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
                        }];
                        [alert addAction:okAction];
                        dispatch_async(dispatch_get_main_queue(), ^(void){
                        ...
                        CheckPassword *vc = [mainStoryboard instantiateViewControllerWithIdentifier:@"NewPassword"];
                        CATransition *transition = [CATransition animation];
                        transition.duration = 0.3;
                        transition.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
                        transition.type = kCATransitionPush;
